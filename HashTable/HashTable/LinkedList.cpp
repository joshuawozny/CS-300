...
void LinkedList::Append(Bid bid, unsigned int key) {
	if (this->idExists(bid.getBidId())) {
		std::cout << "Duplicate key in BidId Enter another bid" << std::endl;
		return;
	}
	//Create new node
	Node* new_node = new Node(bid, key);
	//if there is nothing at the head...
	if (head == nullptr) {
		// new node becomes the head and the tail
		head = new_node;
...
bool LinkedList::idExists(std::string key)
{
	Node* current = head;
